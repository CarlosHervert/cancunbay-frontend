...
<template>
   <div>
      <v-row v-if="!mobile" class="mb-4">
         <v-col
            v-if="video !== null && video !== '' && video !== 'n/a'"
            cols="4"
            class="text-left"
         >
            <div class="iconWrap d-flex mx-2" @click="openVideo">
               <a>
                  <v-icon
                     class="icon2-video generalData mr-2"
                     size="25"
                  ></v-icon>
         ...
         <v-col cols="4" class="text-center">
            <div class="iconWrap d-flex mx-2">
               ...
               <a @click="copyURL">
                  <v-icon
                     class="icon2-share generalData mr-2"
                     size="25"
                  ></v-icon>
         ...
         <v-col cols="4" class="text-right">
            <div class="iconWrap d-flex mx-2">
               <a
                  href="https://api.whatsapp.com/send?phone=529981077585"
                  target="_blank"
               >
                  <v-icon
                     class="icon2-whatsapp generalData mr-2"
                     size="25"
                  ></v-icon>
      ...
      <div class="tour-detail">
         ...
         >
            <p>
               <span class="price-fake d-block"
                  >{{ $t("general.adults") }}:
                  {{ adultFake | currencyFormat(currency) }}</span
               >
               <span class="price-fake d-block"
                  >{{ $t("general.children") }}:
                  {{ childFake | currencyFormat(currency) }}</span
               >
            </p>
            <p class="mt-5">
               <span class="price-real d-block"
                  >{{ $t("general.adults") }}:
                  {{ realAdult | currencyFormat(currency) }}</span
               >
               <span class="price-real d-block"
                  >{{ $t("general.children") }}:
                  {{ realChild | currencyFormat(currency) }}</span
               >
            </p>
         </div>
      ...
      <v-row v-if="mobile">
         <v-col cols="12">
            <div
               class="icons d-flex flex-column flex-wrap justify-center align-stretch"
            >
               <div
                  v-if="video !== null && video !== '' && video !== 'n/a'"
                  class="iconWrap d-flex mx-2"
                  @click="openVideo"
               >
                  <a class="d-flex">
                     <v-icon
...
export default {
   ...
   computed: {
      ...
      adultFake() {
         if (this.$store.getters["booking/language"] === 2) {
            return this.item.is_private
               ? parseFloat(this.item.rates[0].rate_from_fake)
               : this.item.price_fake_adult;
         } else {
            return this.item.is_private
               ? parseFloat(this.item.rates[0].rate_from_fake_mxn)
               : this.item.fake_adult_mxn;
         }
      },
      childFake() {
         if (this.$store.getters["booking/language"] === 2) {
      ...
      realAdult() {
         if (this.$store.getters["booking/language"] === 2) {
            return this.item.is_private
               ? parseFloat(this.item.rates[0].rate_from_real)
               : this.item.price_real_adult;
         } else {
            return this.item.is_private
               ? this.item.rates[0].rate_from_real_mxn
               : this.item.real_adult_mxn;
         }
      },
      realChild() {
         if (this.$store.getters["booking/language"] === 2) {
