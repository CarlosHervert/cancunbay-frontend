<template>
   <div class="d-block links mb-2 text-right" v-if="!mobile">
      ...
      <span v-for="locale in availableLocales" :key="locale.code">
         <a v-if="locale.show" class="mx-2 language" :href="domain">{{
            locale.name
         }}</a>
      </span>

      <nuxt-link :to="localePath({ name: 'contact' })" class="language">
         <span class="mx-2">{{ $t("menu.header.contact") }}</span></nuxt-link
      >
   </div>
...
export default {
   computed: {
      ...
      domain() {
         ...
         return this.$store.getters["booking/language"] === 1
