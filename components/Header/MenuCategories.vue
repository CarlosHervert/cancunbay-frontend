<template>
   <v-container class="py-1" v-if="!mobile">
      <v-row justify="end" align="end" no-gutters>
         <v-col
            :cols="this.$i18n.locale !== 'es' ? '11' : '12'"
            class="text-center"
         >
            <div class="d-inline ml-10">
               <div v-for="(item, i) in items" :key="i" class="d-inline option">
                  ...
                  <nuxt-link
                     v-else
                     :class="['sub2', 'px-4', markLink(item.url)]"
                     :to="
                        localePath({
                           name: 'slug',
                           params: {
         ...
         <v-col sm="12" class="rowMenuMobile">
            <ul class="menu-list pt-2 px-2" v-if="this.$i18n.locale !== 'es'">
               <li class="my-2">
                  <v-btn
                     depressed
                     class="bookBtn2Mobile rounded-lg py-3 align-self-end elevation-8"
                     block
                     @click="openVentrata"
                  >
                     {{ $t("general.book_now") }}
               ...
               <li class="my-2">
                  <nuxt-link
                     ...
                     class="language slim"
                  >
                     {{ $t("menu.header.contact") }}</nuxt-link
                  >
               </li>
               <li class="my-2">
                  <nuxt-link
...
export default {
   ...
   data() {
      return {
         ...
         domain:
            this.language === 1
               ? "https://www.cancunbay.com"
               : "https://www.cancunbay.com.mx",
