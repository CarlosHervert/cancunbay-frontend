      ...
      <v-row>
         ...
         <v-col cols="5" md="4" :class="[!mobile ? 'px-8' : '']">
            <v-btn
               ...
               block
               @click="validatePromcode"
            >
               {{ $t("forms.info.btnPromo") }}</v-btn
            >
         </v-col>
      </v-row>
      <v-row>
   ...
   computed: {
      ...
      nameErrors() {
         ...
         !this.$v.name.required &&
            errors.push(this.$t("forms.info.nameValidation"));
         return errors;
      },
      emailErrors() {
         const errors = [];
      ...
      phoneErrors() {
         ...
         !this.$v.phone.required &&
            errors.push(this.$t("forms.info.phoneValidation"));
         return errors;
      },
      hotelErrors() {
         const errors = [];
         ...
         !this.$v.hotel.required &&
            errors.push(this.$t("forms.info.hotelValidation"));
         return errors;
      },
      language() {
      ...
      totals() {
         if (this.language === "ing")
            return this.$store.getters["booking/bookingTotal"].usd;
      ...
      countrySend() {
         ...
         const aux = this.countries
            .map((element) => element.isoCode)
            .indexOf(country);
         ...
         this.$axios
            ...
            .then((response) => {
               ...
               this.$nuxt.$emit("goPaymentEvent", {
                  client: response.data.data.clientId,
                  total: this.total,
               });
            });
      },
         ...
         if (ban) {
            this.$axios
               .post("getPromoCode", {
                  ...
                  private_rate: this.tourDetail.isPrivate
                     ? this.tourDetail.pax
                     : null,
               })
               .then((response) => {
